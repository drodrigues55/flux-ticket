...
async function runCryptoTest() {
    ...
    const syncTickets = await database_1.prisma.ticket.findMany({
        where: {
    ...
    const finalTicket = await database_1.prisma.ticket.findUnique({
        where: { id: ticket.id },
    });
...
//# sourceMappingURL=test-edge-sync.js.map