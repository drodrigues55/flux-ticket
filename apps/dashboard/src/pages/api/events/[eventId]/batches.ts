...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const response = await fetch(`${apiWriteUrl}/events/${eventId}/batches`, fetchOptions);
    ...
    console.error('[PROXY ERROR] Falha ao encaminhar batches:', error);
    return res.status(500).json({ error: 'Erro interno ao realizar proxy de batches.' });
  }
}
