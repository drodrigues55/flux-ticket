...
export default function EditBatchPage() {
  ...
  const [originalBatch, setOriginalBatch] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const res = await fetch(`/api/organizer/events/${eventId}/ticket-types/${ticketTypeId}/batches/${batchId}`);
        const json = await res.json();
        if (!res.ok) throw json.error || { message: 'Falha ao carregar lote.' };
        ...
        setForm({
          ...
          salesStart: formatDatetime(batch.salesStart),
          salesEnd: formatDatetime(batch.salesEnd),
          purchaseLimit: batch.purchaseLimit || 5,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) {
        throw json.error || { message: 'Falha ao salvar alterações.' };
      }
  ...
  if (loading) {
    return (
      <EventLayout eventId={eventId} eventName={eventName}>
        <div className="p-8 text-sm text-neutral-500">Carregando lote...</div>
      </EventLayout>
    );
  }
    ...
    <EventLayout eventId={eventId} eventName={eventName}>
      <div className="space-y-6 mt-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-neutral-900 mt-2">Editar Lote</h1>
        </div>
